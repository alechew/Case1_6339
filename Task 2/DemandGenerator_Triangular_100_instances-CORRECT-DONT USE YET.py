...
ofile = open(filename + "task2_question2-CHINA-JUST-AVG.csv", "wb")
...
for cityDemand in listOfCities:
    ...
    for l in range(len(cityDemand.yearlyDemands)):
        ...
        ofile.write(title)
        columns = "Year,F10,K10,S10,W10,F20,K20,L20,S20,W20,X20,F30,K30,S30,W30,F50,K50,L50,S50,W50,X50, , ,Year,F10,K10,S10,W10,F20,K20,L20,S20,W20,X20,F30,K30,S30,W30,F50,K50,L50,S50,W50,X50, , ,\n"
        ofile.write(columns)
        ...
        for printDay in range(364):
            for printProduct in range(len(productDictionary)):
                productDayList = []
                for instance in range(100):  # to calculate the average of every instance.
                    productDayList.append(productDictionary.get(printProduct).get("list")[instance])

                average = numpy.average(productDayList)
                averageRow = averageRow + "," + str(int(average))
            ...
            for printProduct in range(len(productDictionary)):
                ...
                for instance in range(100):  # to calculate the average of every instance.
                    productDayList.append(productDictionary.get(printProduct).get("list")[instance])
                std = numpy.std(productDayList)
                averageRow = averageRow + "," + str(int(std))
